...
namespace PRL.f_ChucNang
{
	public partial class f_Voucher : Form
	{
		...
		private void LoadDgv()
		{
			...
			{
				dgv_Voucher.Rows.Add(stt++, item.Idvoucher, item.Code, item.PhanTram,
						...
						if (dtpNgayBatDau.Value.Date <= dtpNgayKetThuc.Value.Date)
						{
							...
							if (txt_MaVocher.Text == search.Code)
							{
								...
								else
								{
									...
									search.PhanTram = Convert.ToDouble(txt_GiamTien.Text);
									...
									search.DieuKienApDung = Convert.ToInt32(txt_DKApDung.Text);
